...
def generate_audio(story_text: str, scene_number: int) -> str | None:
    try:
        audio_file = speak(
            story_text,
            str(MEDIA_DIR / f"scene_{scene_number}.mp3")
        )

        st.success(f"Audio generated successfully: {audio_file}")
        return audio_file
...
st.set_page_config(page_title="Wanna create your own visual novel?", layout="wide")
init_session_state()

st.title("Wanna create your own visual novel?")
